/**
    ...
    public class soClient {
        ...
        // Track which connection was used for file size
        ...
        public long getFileSize(String ip1, int port1, String ip2, int port2, int fileId) throws IOException {
            ...
            if ((response1 != null && response2 == null) || (response1 != null && lossRate1 <= lossRate2)) {
                if (response1.getResponseType() == ResponseType.RESPONSE_TYPES.GET_FILE_SIZE_SUCCESS) {
                    System.out.println("Getting file size from Connection-1 (loss rate: " +
                            String.format("%.2f%%", lossRate1 * 100) + ")");
                    firstConnection = true;
                    return response1.getFileSize();
                }
            } else if (response2 != null) {
                if (response2.getResponseType() == ResponseType.RESPONSE_TYPES.GET_FILE_SIZE_SUCCESS) {
                    System.out.println("Getting file size from Connection-2 (loss rate: " +
                            String.format("%.2f%%", lossRate2 * 100) + ")");
         ...
         */
        public boolean downloadFile(String ip1, int port1, String ip2, int port2, int fileId,
            ...
            // İlk chunk'ı ayır, diğerlerini kuyruğa ekle
            ChunkTask firstChunk = chunkList.get(0);
            for (int i = 1; i < chunkList.size(); i++) {
                queue.add(chunkList.get(i));
            }
    
            System.out.println("Starting download ...");
            startDownloadTime = System.currentTimeMillis();
    
            // İlk chunk'ı filesize'da kullanılan bağlantı ile indir
            try {
                if (firstConnection) {
                    System.out.println("Downloading first chunk using Connection-1 (following filesize connection)");
                    aggregatorDownloadChunk(ip1, port1, fileId, firstChunk.start, firstChunk.end, socket1, stats1);
                } else {
                    System.out.println("Downloading first chunk using Connection-2 (following filesize connection)");
                    aggregatorDownloadChunk(ip2, port2, fileId, firstChunk.start, firstChunk.end, socket2, stats2);
                }
            } catch (Exception e) {
                e.printStackTrace();
                return false;
            ...
            // Kalan chunk'lar için iki thread başlat
